#!/usr/bin/env python3
"""
galaxy_mpi_final_report_ready.py

Final report-ready parallel galaxy / N-body simulation using mpi4py, with Newtonian gravity, Leapfrog integration, runtime CSV export, fixed-frame 2D/3D GIF rendering, and a safe scale-only mode for 10^9-particle discussion.
...
Run examples:
  python galaxy_mpi_final_report_ready.py --allow-serial --particles 250 --years 4000 --dt 25 --plot-interval 1000
  mpiexec -n 4 python galaxy_mpi_final_report_ready.py --particles 800 --years 10000 --dt 25 --plot-interval 1000 --mode report
  mpiexec -n 4 python galaxy_mpi_final_report_ready.py --particles 1000000000 --years 1000000000 --dt 1000 --scale-only --no-plot
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--particles", type=int, default=600, help="Number of stars/particles for the direct N-body run. Use smaller values for laptops.")
    ...
    parser.add_argument("--output-dir", type=str, default="galaxy_final_output", help="Directory for frames, GIF and CSV.")
    parser.add_argument("--gif-name", type=str, default="galaxy_final_report.gif", help="Output GIF filename.")
    ...
    parser.add_argument("--max-3d-points", type=int, default=12_000,
                        help="Maximum number of particles drawn in the 3D subplot per frame. Keeps the GIF legible.")
    parser.add_argument("--density-bins", type=int, default=280,
                        help="Resolution of the 2D density grid used instead of a raw scatter cloud.")
    parser.add_argument("--density-smooth-passes", type=int, default=2,
                        help="Light smoothing passes over the 2D density image. Avoids scipy dependency.")
    parser.add_argument("--overlay-points", type=int, default=5_000,
                        help="Maximum number of crisp points overlaid on the 2D density map.")
    parser.add_argument("--camera-azim-start", type=float, default=25.0,
                        help="Initial 3D camera azimuth in degrees.")
    parser.add_argument("--camera-azim-end", type=float, default=115.0,
                        help="Final 3D camera azimuth in degrees.")
    parser.add_argument("--camera-elev", type=float, default=24.0,
                        help="Fixed 3D camera elevation in degrees.")
...
def smooth_density(field: np.ndarray, passes: int) -> np.ndarray:
    """Small separable blur for the 2D density map; avoids requiring scipy."""
    if passes <= 0:
        return field
...
def draw_core_glow_2d(ax2d) -> None:
    """Three glow layers plus a crisp central point."""
    ax2d.scatter([0], [0], s=3200, c="#ff7a00", alpha=0.040, linewidths=0, zorder=8)
    ax2d.scatter([0], [0], s=1250, c="#ffd166", alpha=0.115, linewidths=0, zorder=9)
    ax2d.scatter([0], [0], s=290, c="#fff0b3", alpha=0.96, edgecolors="#ffb347", linewidths=0.8, zorder=10)
    ax2d.scatter([0], [0], s=50, c="#ffffff", alpha=1.0, linewidths=0, zorder=11)


def draw_core_glow_3d(ax3d) -> None:
    ax3d.scatter([0], [0], [0], s=1000, c="#ff7a00", alpha=0.050, linewidths=0)
    ax3d.scatter([0], [0], [0], s=360, c="#ffd166", alpha=0.160, linewidths=0)
    ax3d.scatter([0], [0], [0], s=110, c="#fff0b3", alpha=1.0, edgecolors="#ffb347", linewidths=0.8)
    ax3d.scatter([0], [0], [0], s=28, c="#ffffff", alpha=1.0, linewidths=0)
def render_frame(
    ...
    """Render one fixed-size, report-ready frame with 2D density and clean 3D sample.

    The canvas, axis limits, text placement and save options are intentionally fixed.
    This avoids the common GIF problem where successive PNG frames have slightly
    ...
    # Fixed limits for every frame. The visualization is allowed to show particles
    # escaping the main galaxy; the axes themselves never change.
    extent = args.galaxy_radius * 1.45
    ...
    nebula_cmap = LinearSegmentedColormap.from_list(
        "deep_space_nebula_final",
        ["#02030a", "#061029", "#12175a", "#321069", "#79287d", "#ff9f1c"],
    )
    ...
    grid = fig.add_gridspec(1, 2, width_ratios=[1.08, 0.92], wspace=0.075)
    ...
    # Decorative but fixed background. It does not change between frames.
    ax2d.imshow(
        nebula,
    ...
    ax2d.scatter(bx[::7], by[::7], s=bs[::7] * 8.0, c="#9fc5ff", alpha=0.075, linewidths=0, zorder=1)
    ax3d.scatter(bx, by, bz, s=bs * 0.65, c="#ffffff", alpha=0.11, linewidths=0)

    # A) + B) 2D density image instead of raw heavy scatter.
    hist, xedges, yedges = np.histogram2d(
        pos[:, 0], pos[:, 1],
        bins=int(args.density_bins),
        range=[[-extent, extent], [-extent, extent]],
    )
    hist = smooth_density(hist, int(args.density_smooth_passes))
    visible = hist[hist > 0]
    vmax = max(float(np.percentile(visible, 99.4)), 2.0) if visible.size else 2.0
    ax2d.imshow(
        hist.T + 1.0,
    ...
    # Trails are sampled so the frame stays readable and fast.
    rng = np.random.default_rng(args.seed + 99_991 + frame_index)
    if len(history) > 1 and args.trail_length > 0:
        hist_list = list(history)
        max_segments = min(len(hist_list) - 1, int(args.trail_length))
        trail_n = min(max(400, int(args.overlay_points)), n)
        trail_idx = rng.choice(n, size=trail_n, replace=False) if n > trail_n else np.arange(n)
        for h in range(max_segments):
            prev = hist_list[-max_segments + h]["pos"][trail_idx, :2]
            curr = hist_list[-max_segments + h + 1]["pos"][trail_idx, :2]
            segments = np.stack([prev, curr], axis=1)
            age = (h + 1) / max_segments
            alpha = 0.020 + 0.125 * age
            width = 0.16 + 0.42 * age
            lc = LineCollection(segments, colors=(0.48, 0.78, 1.0, alpha), linewidths=width, zorder=3)
            ax2d.add_collection(lc)

    # Crisp overlay: a few points only, not the entire dataset.
    inner = np.percentile(radius, 4)
    outer = np.percentile(radius, 97)
    ...
    sizes = np.clip(4.0 + 18.0 * mass_norm + 6.0 * speed_norm, 3.0, 32.0)

    overlay_n = min(int(args.overlay_points), n)
    overlay_idx = rng.choice(n, size=overlay_n, replace=False) if n > overlay_n else np.arange(n)
    ax2d.scatter(
        pos[overlay_idx, 0], pos[overlay_idx, 1],
        s=sizes[overlay_idx] * 4.0,
        c=color_values[overlay_idx],
        cmap=star_cmap,
        alpha=0.065,
        linewidths=0,
        zorder=4,
    ...
    # C) 3D uses fewer points than 2D.
    n3 = min(int(args.max_3d_points), n)
    idx3 = rng.choice(n, size=n3, replace=False) if n > n3 else np.arange(n)
    floor_z = -extent * 0.38
    ax3d.scatter(
        pos[idx3, 0], pos[idx3, 1], np.full(n3, floor_z),
        s=sizes[idx3] * 0.15,
        c=color_values[idx3], cmap=star_cmap,
        alpha=0.060,
        linewidths=0,
    )
    ax3d.scatter(
        pos[idx3, 0], pos[idx3, 1], pos[idx3, 2],
        s=sizes[idx3] * 0.70,
        c=color_values[idx3], cmap=star_cmap,
        alpha=0.82,
        linewidths=0,
    ...
    # Fixed axes and text: no tight_layout, no bbox_inches="tight".
    ax2d.set_xlim(-extent, extent)
    ax2d.set_ylim(-extent, extent)
    ax2d.set_aspect("equal", adjustable="box")
    ax2d.set_title("2D density view — sampled N-body galaxy", color="white", fontsize=13, pad=10)
    ax2d.set_xlabel("x", color="#cfd8ff")
    ax2d.set_ylabel("y", color="#cfd8ff")
    ax2d.tick_params(colors="#8fa0d6", labelsize=8)
    ax2d.grid(color="#27395f", alpha=0.20, linewidth=0.55)

    ax3d.set_xlim(-extent, extent)
    ax3d.set_ylim(-extent, extent)
    ax3d.set_zlim(-extent * 0.40, extent * 0.40)
    ax3d.set_title("3D sample view — fixed camera path", color="white", fontsize=13, pad=10)
    ...
    expected_frames = getattr(args, "expected_frames", max(2, frame_index + 1))
    camera_progress = frame_index / max(1, int(expected_frames) - 1)
    azim = args.camera_azim_start + (args.camera_azim_end - args.camera_azim_start) * camera_progress
    ax3d.view_init(elev=args.camera_elev, azim=azim)
    for axis in (ax3d.xaxis, ax3d.yaxis, ax3d.zaxis):
        axis.pane.set_facecolor((0.02, 0.02, 0.07, 0.16))
        axis.pane.set_edgecolor((0.25, 0.28, 0.45, 0.22))
    ax3d.grid(True)

    title = "Parallel Galaxy Simulation — report-ready N-body/MPI version"
    st = fig.suptitle(title, color="#f8fbff", fontsize=17, y=0.985, fontweight="bold")
    st.set_path_effects([path_effects.withStroke(linewidth=3, foreground="#02030a")])

    hud = (
        f"year {year:,.0f}   •   N={n}   •   MPI ranks={mpi_info['size']}   •   frame {frame_index + 1}/{expected_frames}\n"
    ...
    fig.text(
        ...
        fontsize=9.5,
        family="monospace",
        bbox=dict(boxstyle="round,pad=0.48", facecolor="#05091c", edgecolor="#37508a", alpha=0.72),
    )
    fig.text(
        0.605, 0.035,
        ...
        color="#b7c8ff",
        fontsize=8.7,
        bbox=dict(boxstyle="round,pad=0.33", facecolor="#05091c", edgecolor="#283a68", alpha=0.58),
    )
...
def create_gif(frame_paths: List[Path], gif_path: Path, fps: int) -> None:
    """Create a GIF, enforcing equal frame sizes as an extra safety net."""
...
def run_scale_only(args: argparse.Namespace, rank: int, size: int, MPI: Any, comm: Any) -> None:
    ...
    if rank == 0:
        ...
        print("=== Parallel Galaxy Simulation: HPC scale-only estimate ===")
...
def main() -> None:
    ...
    if rank == 0:
        print("=== Parallel Galaxy Simulation: final report-ready edition ===")
        ...
        print("Integrator: Newtonian gravity + Leapfrog / velocity-Verlet kick-drift-kick")
    ...
    # Number of frames expected in the normal simulation. Used only for a smooth, fixed camera path.
    args.expected_frames = len([s for s in range(steps + 1) if (s % plot_every_steps == 0 or s == steps)])
